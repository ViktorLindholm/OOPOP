...
simpsons(males) ->
 [Name ||{_,Male,Name} <- simpsons(), Male =:= male];
simpsons(females) ->
 [Name ||{_,Female,Name} <- simpsons(), Female =:= female];
simpsons(pets) ->
 [Name ||{Pets,_,Name} <- simpsons(), Pets =/= person].
...
char_to_upper(Char) when Char =< 122 , Char >= 97 ->
    Char-32;
char_to_upper(Char) ->
    Char.
...
char_to_lower(Char) when Char =< 90, Char >= 65 ->
    Char+32;
char_to_lower(Char)->
    Char.
